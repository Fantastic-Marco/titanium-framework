...
public final class Types {
    ...
    public static boolean isNumeric(Class<?> elementClass) {
        // 不允许在 vo 实体使用 AtomicInteger 或 AtomicLong 等类型, 这用法有点奇怪
        return Types.isByte(elementClass)
                || Types.isShort(elementClass)
                || Types.isInt(elementClass)
                || Types.isLong(elementClass)
                || Types.isFloat(elementClass)
                || Types.isDouble(elementClass)
                || Types.isInstanceOf(elementClass, BigInteger.class, BigDecimal.class);
    }

    public static boolean isInteger(Object value) {
        return Types.isByte(value)
                || Types.isShort(value)
                || Types.isInt(value)
                || Types.isLong(value)
                || Types.isInstanceOf(value.getClass(), BigInteger.class);
    }
